...
PROJECTS = {
    "honey": {
        ...
        "product": {
            ...
            "net_content": "250 g",
            "ingredients": "Mật ong hoa cà phê Đắk Lắk nguyên chất, vị ngọt dịu, hậu hương thơm ấm.",
    ...
    "matcha": {
        ...
        "product": {
            ...
            "net_content": "100 g",
            "ingredients": "Lá trà xanh Thái Nguyên nghiền mịn, màu xanh tự nhiên, hương vị thanh dịu.",
...
def write_project(kind: str, out: Path) -> Path:
    ...
    base["branding"] = {
        "logos": [{"asset_id": "brand", "path": str((BASE / "assets/brand.png").resolve()), "asset_type": "png", "role": "brand"}],
        "ocop": {"logo_path": str((BASE / "assets/ocop.png").resolve()), "star_count": 3},
    }
    base["packaging"] = {"size_id": "OCOP_130X150", "qr_payload": f"https://example.com/ocop/{base['project_id']}"}
...
def main() -> None:
    ...
    for kind in PROJECTS:
        ...
        manifest = LayoutManifest(run_id=f"phase4_{kind}_acceptance", project_id=project.project_id, dieline_version=dieline.version, candidate=candidate)
        ...
        pix.save(raster)
        doc.close()
        typography = {k: v.model_dump(mode="json") for k, v in resolve_candidate_typography(project, candidate, dpi=150).items()}
        qa = qa_candidate(manifest.run_id, project, dieline, candidate, png)
        (run / "selected_candidate.json").write_text(candidate.model_dump_json(indent=2), encoding="utf-8")
        (run / "layout_manifest.json").write_text(manifest.model_dump_json(indent=2), encoding="utf-8")
        (run / "typography_metadata.json").write_text(json.dumps(typography, ensure_ascii=False, indent=2), encoding="utf-8")
        ...
        review = "Print packaging.pdf at 100% / actual size. Disable fit-to-page/scaling. Verify Vietnamese accents, side-panel readability, and physical 1:1 dimensions with a ruler.\n"
        (run / "PRINT_1_TO_1_REVIEW.md").write_text(review, encoding="utf-8")
        manifest_hashes = {p.name: sha(p) for p in sorted(run.iterdir()) if p.is_file() and p.name != "artifact_sha256_manifest.json"}
        (run / "artifact_sha256_manifest.json").write_text(json.dumps(manifest_hashes, indent=2), encoding="utf-8")
